...
export class FakturadetaljerPage {
    private page: Page;
    private contactButton = () => this.page.getByRole('button', {name: 'Kontakt (valgfri ved kvittering)'});
    private accountButton = () => this.page.getByRole('button', {name: 'Konto *'});
    private bokforButton = () => this.page.locator('[class*=justify-space-between] button[kb-nav]');
    private amountField = () => this.page.getByRole('textbox', {name: 'Totalt beløp inkl. mva. *'});
    private invoiceNumberField = () => this.page.getByRole('textbox', {name: 'Fakturanr.'});
    private invoiceDateField = () => this.page.locator('#input-164');
    private dueDateField = () => this.page.locator('#input-168');
    private successPopUp = () => this.page.getByText('Bilag opprettet med bilagsnr.');
    private invoiceNumberErrorMessage = () => this.page.getByText('Fakturanr. er allerede bokført');
    ...
    public async goto() {
        await this.page.goto(process.env.BASE_URL + 'systimaas7/bookkeeping/purchase');
    }
    ...
    public async assertInvoiceNumberErrorMessageIsVisibleAndRed() {
        ...
        expect(await getBackgroundColor(this.invoiceNumberErrorMessage())).toBe('rgb(255, 82, 82)');
    }
}